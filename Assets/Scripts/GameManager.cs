...
public class GameManager : MonoBehaviour
{
    ...
    private void Initialize(GameSettings gameSettings)
    {
        ...
        if (gameSettings.flushDraw)
        {
            ...
            communityCards = BluffCases.FlushDrawCc(ref deck, gameSettings.communityCardSize);
            user = new("You", BluffCases.FlushDrawP(ref deck, communityCards), gameSettings.userStartingMoney);
        ...
        else if (gameSettings.gutShot)
        {
            ...
            List<Player> players = new();
            communityCards = BluffCases.StraightDrawCc(ref deck, gameSettings.communityCardSize);
            user = new("You", BluffCases.GutShotP(ref deck, communityCards), gameSettings.userStartingMoney);
            Utilities.ShowCards(ref communityCards);
            players.Add(new(Utilities.RandomName(), Utilities.DeckTakeTwo(ref deck), gameSettings.playerStartingMoney));
            game = new(gameSettings.numberOfPlayers, gameSettings.startingPotSize, deck, communityCards, user, gameSettings.userPosition, players);
        }

        else if (gameSettings.underPair)
        {
            List<Card> deck = Utilities.NewDeck();
            deck.Shuffle();
            List<Card> communityCards = null;
            Player user = null;
            List<Player> players = new();
            communityCards = BluffCases.UnderPairCc(ref deck, gameSettings.communityCardSize);
            user = new("You", BluffCases.UnderPairP(ref deck, communityCards), gameSettings.userStartingMoney);
            Utilities.ShowCards(ref communityCards);
            players.Add(new(Utilities.RandomName(), Utilities.DeckTakeTwo(ref deck), gameSettings.playerStartingMoney));
            game = new(gameSettings.numberOfPlayers, gameSettings.startingPotSize, deck, communityCards, user, gameSettings.userPosition, players);
        }
        else if (gameSettings.overPair)
        {
            List<Card> deck = Utilities.NewDeck();
            deck.Shuffle();
            List<Card> communityCards = null;
            Player user = null;
            List<Player> players = new();
            communityCards = BluffCases.OverPairCc(ref deck, gameSettings.communityCardSize);
            user = new("You", BluffCases.OverPairP(ref deck, communityCards), gameSettings.userStartingMoney);
            Utilities.ShowCards(ref communityCards);
            players.Add(new(Utilities.RandomName(), Utilities.DeckTakeTwo(ref deck), gameSettings.playerStartingMoney));
            game = new(gameSettings.numberOfPlayers, gameSettings.startingPotSize, deck, communityCards, user, gameSettings.userPosition, players);
        }
        else if (gameSettings.straightFlushDraw)
        {
            List<Card> deck = Utilities.NewDeck();
            deck.Shuffle();
            List<Card> communityCards = null;
            Player user = null;
            List<Player> players = new();
            communityCards = BluffCases.StraightFlushDrawCc(ref deck, gameSettings.communityCardSize);
            user = new("You", BluffCases.StraightFlushDrawP(ref deck, communityCards), gameSettings.userStartingMoney);
            Utilities.ShowCards(ref communityCards);
            players.Add(new(Utilities.RandomName(), Utilities.DeckTakeTwo(ref deck), gameSettings.playerStartingMoney));
            game = new(gameSettings.numberOfPlayers, gameSettings.startingPotSize, deck, communityCards, user, gameSettings.userPosition, players);
        }
        ...
        List<Card> cards = new(game.CommunityCards);
        cards.AddRange(new List<Card>(game.User.Cards));

        List<BluffCases.BluffCase> bluffCases = BluffCases.GetAllBluffCases(cards);
